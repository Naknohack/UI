...
QuestNeta = function()
	...
	local BananaLib = loadstring(game:HttpGet("https://raw.githubusercontent.com/Naknohack/UI/refs/heads/main/ui%20BananaHub.lua"))()
local Library = BananaLib:CreateWindow({
    Title = "Nak Hub V4",
    Desc = "by Naknohack",
...
function Library:SetScale(_)
    return true
end

function Library:Minimize(state)
    local gui = game:GetService("CoreGui"):FindFirstChild("Night Mystic GUI")
    if gui then
        gui.Enabled = not state
    end
    getgenv().UIToggled = not state
end

function Library:MakeTab(tabInfo)
    if type(tabInfo) == "table" then
        local title = tabInfo.Title or tabInfo.Name or ""
        local icon = tabInfo.Icon or tabInfo.Image or ""
        if type(icon) == "string" and icon ~= "" and not icon:find("rbxassetid://", 1, true) and not icon:match("^%d+$") then
            icon = ""
        end

        local tab = self:AddTab(title, icon)
        if type(tab) == "table" and tab.AddSection then
            local oldAddSection = tab.AddSection
            function tab:AddSection(sectionInfo, ...)
                if type(sectionInfo) == "table" then
                    sectionInfo = sectionInfo[1] or sectionInfo.Text or sectionInfo.Title or ""
                end
                return oldAddSection(self, sectionInfo, ...)
            end
        end
        return tab
    end

    local tab = self:AddTab(tostring(tabInfo or ""), "")
    return tab
end
