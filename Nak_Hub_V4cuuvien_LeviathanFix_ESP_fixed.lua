...
Event:AddToggle({
	...
	Callback = function(I)
		...
		task.spawn(function()
			...
			local function AttackSegment(part)
				if not part or not _G.Leviathan1 then return end

				local char = plr.Character
				local hrp = char and char:FindFirstChild("HumanoidRootPart")
				if not hrp then return end

				-- Bám vào khúc thân để đánh
				hrp.CFrame = part.CFrame * CFrame.new(0, 18, 0)
				task.wait(0.12)

				-- Dùng đúng skill set đang dùng ở Tyrant Of The Skies
				pcall(function()
					UseAllCategorySkills()
				end)
			end
			...
			while task.wait(0.35) do
				...
				local Levi = SeaBeasts and SeaBeasts:FindFirstChild("Leviathan")

				if Levi and Levi.Parent then
					-- Ưu tiên khúc thân trước, rồi mới đến đầu
					for _, part in ipairs(Levi:GetChildren()) do
						if not _G.Leviathan1 then break end
						if part:IsA("BasePart") and (
							string.find(part.Name, "Segment")
							or string.find(part.Name, "Body")
							or part.Name == "Head"
						) then
							AttackSegment(part)
							task.wait(0.15)
					...
					if Levi:FindFirstChild("Humanoid") and Levi.Humanoid.Health <= 0 then
						_G.Leviathan1 = false
					end
				else
					_G.Leviathan1 = false
...
local function FormatMeters(dist)
	...
	return string.format("%.0f M", math.max(dist, 0))
end

Number = math.random(1, 1000000);

-- ESP DE PLAYERS (BRANCO) - NOME / LEVEL / HP / DISTÂNCIA
EspPly = function()
    local myChar = plr.Character
    local myHrp = myChar and myChar:FindFirstChild("HumanoidRootPart")

    for _, e in next, game.Players:GetPlayers() do
        if e ~= plr then
            pcall(function()
                if not isnil(e.Character) and e.Character:FindFirstChild("Head") and e.Character:FindFirstChild("Humanoid") then
                    local head = e.Character.Head
                    local hum = e.Character.Humanoid
                    local hrp = e.Character:FindFirstChild("HumanoidRootPart") or e.Character:FindFirstChild("Torso")
                    local distText = "0 M"
                    if myHrp and hrp then
                        distText = FormatMeters((myHrp.Position - hrp.Position).Magnitude)
                    end

                    local hpText = string.format("HP: %d/%d", math.floor(hum.Health), math.floor(hum.MaxHealth))
                    local labelText = string.format("%s | Lv. %s | %s | %s", e.Name, tostring(e.Data.Level.Value), hpText, distText)

                    if PlayerEsp then
                        local bb = head:FindFirstChild("NameEsp" .. Number)
                        if not bb then
                            bb = Instance.new("BillboardGui")
                            bb.Name = "NameEsp" .. Number
                            bb.ExtentsOffset = Vector3.new(0, 1, 0)
                            bb.Size = UDim2.new(1, 240, 1, 36)
                            bb.Adornee = head
                            bb.AlwaysOnTop = true
                            bb.Parent = head

                            local K = Instance.new("TextLabel", bb)
                            K.Name = "TextLabel"
		...
		pcall(function()
			if island.Name ~= "Sea" then
				if _G.IslandESP then
					local bb = island:FindFirstChild("IslandESP")
					if not bb then
						bb = Instance.new("BillboardGui")
						bb.Name = "IslandESP"
						bb.Size = UDim2.new(0, 170, 0, 40)
						bb.StudsOffset = Vector3.new(0, 35, 0)
						bb.AlwaysOnTop = true
						bb.Parent = island
					...
					if label then
						local distText = "0 M"
						if myHrp and island:IsA("BasePart") then
							distText = FormatMeters((myHrp.Position - island.Position).Magnitude)
						elseif myHrp and island:IsA("Model") and island.PrimaryPart then
							distText = FormatMeters((myHrp.Position - island.PrimaryPart.Position).Magnitude)
						end
						label.Text = island.Name .. " | " .. distText
					end
				else
...
local function StartESPLoop(flagName, interval, callback, cleanup)
    local runningKey = flagName .. "_Running"
    if _G[runningKey] then return end
    _G[runningKey] = true
...
Esp:AddToggle({Name = "Esp Berries", Default = false, Callback = function(I)
    BerryEsp = I
    if I then
        StartESPLoop("BerryEsp", 1, berriesEsp, function()
            pcall(berriesEsp)
        end)
    else
        pcall(berriesEsp)
...
Esp:AddToggle({Name = "Esp Players", Default = false, Callback = function(I)
    PlayerEsp = I
    if I then
        StartESPLoop("PlayerEsp", 0.1, EspPly, function()
            pcall(EspPly)
        end)
    else
        pcall(EspPly)
...
Esp:AddToggle({Name = "Esp Chests", Default = false, Callback = function(I)
    ChestESP = I
    if I then
        StartESPLoop("ChestESP", 1, ChestEsp, function()
            pcall(ChestEsp)
        end)
    else
        pcall(ChestEsp)
...
Esp:AddToggle({Name = "Esp Fruits", Default = false, Callback = function(I)
    DevilFruitESP = I
    if I then
        StartESPLoop("DevilFruitESP", 1, DevEsp, function()
            pcall(DevEsp)
        end)
    else
        pcall(DevEsp)
...
Esp:AddToggle({Name = "Esp Island", Default = false, Callback = function(I)
    _G.IslandESP = I
    if I then
        StartESPLoop("IslandESP", 2, IslandESP_Func, function()
...
if World2 then
    Esp:AddToggle({Name = "Esp Flower", Default = false, Callback = function(I)
        FlowerESP = I
        if I then
            StartESPLoop("FlowerESP", 1, flowerEsp, function()
                pcall(flowerEsp)
            end)
        else
            pcall(flowerEsp)
    ...
    Esp:AddToggle({Name = "Esp Legendary Sword", Default = false, Callback = function(I)
        LegenS = I
        if I then
            StartESPLoop("LegenS", 1, LegenSword, function()
                pcall(LegenSword)
            end)
        else
            pcall(LegenSword)
...
if World2 or World3 then
    Esp:AddToggle({Name = "Esp Aura Colour Dealers", Default = false, Callback = function(I)
        ColorEsp = I
        if I then
            StartESPLoop("ColorEsp", 1, HakiClorEsp, function()
                pcall(HakiClorEsp)
            end)
        else
            pcall(HakiClorEsp)
...
if World3 then
    Esp:AddToggle({Name = "Esp Gears", Default = false, Callback = function(I)
        ESPGear = I
        if I then
            StartESPLoop("ESPGear", 1, gearEsp, function()
                pcall(gearEsp)
            end)
        else
            pcall(gearEsp)
    ...
    Esp:AddToggle({Name = "Esp Advanced Fruits Dealer", Default = false, Callback = function(I)
        advanEsp = I
        if I then
            StartESPLoop("advanEsp", 1, AdvanFruitEsp, function()
                pcall(AdvanFruitEsp)
            end)
        else
            pcall(AdvanFruitEsp)
