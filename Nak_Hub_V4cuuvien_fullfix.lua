	...
	Callback = function(I)
		...
		task.spawn(function()
			...
			while _G.Leviathan1 do
				...
				local LeviHum = Levi:FindFirstChildOfClass("Humanoid")
				if LeviHum and LeviHum.Health <= 0 then
					_G.Leviathan1 = false
					break
				end
				...
				-- Bám từng khúc thân, không chỉ bay quanh đầu
				for _, v in ipairs(Levi:GetDescendants()) do
					if not _G.Leviathan1 then
						break
					end

					if v:IsA("BasePart") and (string.find(v.Name, "Segment") or string.find(v.Name, "Body") or v.Name == "Head") then
						AttackPart(v)
						task.wait(0.12)
					end
				end

				if Levi:FindFirstChild("Head") then
					AttackPart(Levi.Head)
				end
